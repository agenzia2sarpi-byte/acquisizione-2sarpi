...
function vistaRapporto() {
  ...
  <div class="foglio" id="foglioRapporto">
    ...
    <div class="firma">
      <b>${esc(persona(S.operatore).agenzia)}</b>${persona(S.operatore).insieme ? " in collaborazione con " + esc(persona(S.operatore).insieme) : ""} — ${esc(persona(S.operatore).nome)} · ${esc(persona(S.operatore).telefono)}${persona(S.operatore).telefono2 ? " / " + esc(persona(S.operatore).telefono2) : ""}<br>
