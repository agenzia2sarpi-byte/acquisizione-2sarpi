...
function render() {
  ...
  <div class="scheda"><h3>Fuori dalla lista <span class="etichetta">${(S.esclusi || []).length} in archivio</span></h3>
    <p style="font-family:var(--serif);font-size:14.5px">Chi finisce qui non torna piu'. Ci arrivano le agenzie, gli immobili scartati e <b>quelli gia' contattati che non hanno portato a niente</b> — cosi' nessuno dei tre richiama un numero che un altro ha gia' chiamato. Non e' un filtro di visualizzazione: l'immobile esce dall'archivio, e a ogni aggiornamento viene riconosciuto e respinto <b>anche se il portale lo ripubblica con un altro indirizzo, un altro numero o il testo cambiato</b> — il confronto e' su indirizzo, metratura, recapito, fotografie e testo, non sul collegamento. Se un'esclusione era sbagliata, si rimette in lista da qui — oppure, piu' comodo, dalla linguetta <b>Archivio</b> in fondo alla riga delle linguette della pagina <a href="radar.html">Annunci</a>: e' lo stesso archivio visto dal posto dove si lavora.</p>
